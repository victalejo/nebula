...
func (r *DeploymentRepository) Create(ctx context.Context, deployment *storage.Deployment) error {
	query := `
		INSERT INTO deployments (id, app_id, version, slot, status, source_config, environment, error_message, created_at, started_at, finished_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
...
func (r *DeploymentRepository) GetByID(ctx context.Context, id string) (*storage.Deployment, error) {
	query := `
		SELECT id, app_id, version, slot, status, source_config, environment, error_message, created_at, started_at, finished_at
		FROM deployments
		WHERE id = ?
	`
